...
which is a number with 50 digits. It is unlikely that a laptop or a desktop will be able to store each digit in memory. Instead some digits will be discarded and the result of the test will be misleading. Using my macbook pro, and restricting the range for bases to (1,5) I was able to correctly get prime numbers up to 67. Beyond that my memory is not enough to carry out the test.
